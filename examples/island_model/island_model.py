...
def viz_plots(problems, modulo):
    ...
    for i, p in enumerate(problems):
        ...
        tp = probe.CartesianPhenotypePlotProbe(
            contours=p, xlim=p.bounds, ylim=p.bounds, modulo=modulo, ax=plt.gca()
        )
    ...
    plt.subplots_adjust(
        left=0.05, bottom=0.05, right=0.95, top=0.95, wspace=0.2, hspace=0.3
    )
...
if __name__ == "__main__":
    ...
    print(
        f"[bold magenta]Running island model example "
        f"for {generations} generations "
        f"({'test mode' if test_mode else 'normal mode'})[/bold magenta]"
    )
    ...
    with open("./example_migration_data.csv", "w") as migration_file:
        ea = multi_population_ea(
            ...
            shared_pipeline=[
                ...
                ops.clone,
                mutate_gaussian(
                    std=30, expected_num_mutations=1, bounds=problem.bounds
                ),
                ops.evaluate,
                ...
                ops.migrate(
                    ...
                    migration_gap=50,
                    metric=ops.migration_metric(stream=migration_file, header=True),
                ),
